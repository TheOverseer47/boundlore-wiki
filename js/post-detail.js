...
function renderPost(post) {
  ...
  if (currentUserId && post.author_id === currentUserId) {
    ...
    editBtn.href = `/wiki/edit-post/?slug=${post.slug}`;
  }
}
...
async function handleReaction(postId, reaction) {
  ...
  if (existing && existing.reaction === reaction) {
    ...
    await supabase.from("post_reactions").insert({ post_id: postId, user_id: currentUserId, reaction });
  }
...
function renderComment(c) {
  ...
  return `
    <div class="comment-item" id="comment-${c.id}" style="border-bottom:1px solid #222;padding:12px 0;">
      <div class="comment-meta" style="color:var(--text-muted);font-size:0.9em;">
