...
def main() -> None:
    ...
    check("Resolve-StorageChildFailure" in live, "storage child failure resolver")
    check("$null = $pSt.StandardError.ReadToEnd()" not in live, "storage stderr not discarded")
    check("Protect-StorageDiagnosticText" in live or "StorageExportDiagnostics.ps1" in live, "storage diagnostic redaction lib")
    check("UploadDiagnostics.ps1" in live, "upload diagnostics lib sourced")
    check("Resolve-UploadFailure" in live, "upload failure resolver")
    check("--s3-no-head" in live, "write-only s3-no-head")
    check('"--immutable"' not in live.split("# Wasabi upload once")[-1].split("REMOTE_SIZE_VERIFICATION_PASS")[0], "immutable removed from upload args")
    check("STOP_UPLOAD_DIAGNOSTIC_UNCLASSIFIED" in stops, "upload unclassified stop code")
    check('Stop-Code "STOP_REMOTE_UPLOAD_NOT_AUTHORIZED"' not in live, "legacy upload stop not primary")
    ...
    check("ManualWasabiUpload" in text, "manual wasabi upload switch present")
    check("ManualWasabiUpload conflicts with AllowExternalWasabiUpload" in text, "manual/auto conflict guard")
